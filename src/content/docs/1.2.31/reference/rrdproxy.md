...
which settings control it. All of it is read from `lib/rrd.php`. Nothing here was read from the daemon's source, so where the daemon's behaviour
...
See [Configuration](/1.2.31/reference/configuration/) for where these sit among the rest.
...
[RRDtool integration](/1.2.31/reference/rrdtool-integration/).
...
[Remote data collection](/1.2.31/concepts/remote-data-collection/): collection distributes and
storage does not.
