...
for (const [page, entry] of Object.entries(manifest.pages)) {
	...
	assert.equal(createHash('sha256').update(source).digest('hex'), entry.sourceSha256, `English source changed; review translation: ${page}`);
}
console.log(`${Object.keys(locales).length} locales: routing, direction, navigation, fallback and ${Object.keys(manifest.pages).length} translation sources checked`);
