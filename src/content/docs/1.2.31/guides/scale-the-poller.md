...
intended behaviour and the settings that control it,
:::
...
Read [Architecture](/1.2.31/concepts/architecture/) first if you are not clear on which
...
* The buffer table is durable, not in-memory, so unflushed samples survive a
  ...
  [Back up and restore](/1.2.31/guides/back-up-and-restore/) before you snapshot
  anything.
