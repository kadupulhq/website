...
description: Every script in the cli directory, what it does, and the arguments
  the most useful ones accept.
banner:
...
`--list-query-types --snmp-query-id=ID`, `--list-snmp-fields --host-id=ID [--snmp-query-id=ID]`, `--list-snmp-values --host-id=ID [--snmp-query-id=ID] --snmp-field=Field`.
