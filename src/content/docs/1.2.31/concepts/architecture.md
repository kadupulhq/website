...
slug: 1.2.31/concepts/architecture
---
...
4. A graph image is assembled into an RRDtool command from database configuration,
   ...
   [How graphs are drawn](/1.2.31/concepts/how-graphs-are-drawn/).
...
5. A pass reads that table, groups the values by file, and issues one RRDtool
   update per file. See [High volume writes](/1.2.31/concepts/high-volume-writes/).
6. Device status and countdowns go back to the database.

The ordering of a full run, with its overrun guards, is in
[Poller lifecycle](/1.2.31/reference/poller-lifecycle/).
...
[The poller cache](/1.2.31/concepts/the-poller-cache/).
...
| Configuration change had no effect | Poller cache | [The poller cache](/1.2.31/concepts/the-poller-cache/) |
| Graph has small regular gaps | Poller overrunning its interval, or a heartbeat set too close to the step | [Time and intervals](/1.2.31/concepts/time-and-intervals/) |
| Graph draws the wrong shape from correct data | Graph definition, not collection | [How graphs are drawn](/1.2.31/concepts/how-graphs-are-drawn/) |
| Ports renumbered and history followed the wrong one | Data query index choice | [Data queries and indexes](/1.2.31/concepts/data-queries-and-indexes/) |
| Account sees the console but no graphs | Object permissions, not realms | [Permissions and access](/1.2.31/concepts/permissions-and-access/) |
| Change took effect on one collector and not another | Remote collector out of sync | [Remote data collection](/1.2.31/concepts/remote-data-collection/) |
| Everything slow under load | Database, or the poller not finishing inside its interval | [High volume writes](/1.2.31/concepts/high-volume-writes/) |
